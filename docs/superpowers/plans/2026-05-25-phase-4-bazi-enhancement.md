  ...
  for (const { pillar, pillarLabel } of allPillars) {
    if (pillar.stem === tianDeMap[monthBranch] || pillar.branch === tianDeMap[monthBranch]) {
      results.push({
        ...
        source: '月支',
        pillar: pillarLabel,
        position: pillar.stem === tianDeMap[monthBranch] ? '天干' : '地支',
      ...
      <div v-if="auspicious.length > 0">
        ...
        <div class="flex flex-wrap gap-1.5">
          ...
          >
            ...
            >
              {{ ss.description }}
              <span class="block mt-0.5 opacity-60 text-[0.65rem]"
                >{{ ss.source }} · {{ ss.pillar }}{{ ss.position }}</span
              >
            </span>
          </span>
        </div>
      ...
      <div v-for="(year, idx) in years" :key="year.year">
        ...
        >
          <div class="flex items-center gap-2.5">
            ...
            >
            <span
              class="font-sans text-xs px-1.5 py-0.5 rounded"
              :style="{
                background: year.isFavorable ? '#4A7C5918' : '#C628280E',
                color: year.isFavorable ? '#4A7C59' : '#C6282890',
              }"
              >{{ year.tenGod }}</span
            >
            <!-- Score bar -->
            ...
            <span
              class="font-sans text-xs font-medium w-8 text-right"
              :style="{ color: scoreColor(year.score) }"
              >{{ year.score }}</span
            >
          </div>
          ...
          >
            <div v-if="year.earthRelations.length > 0">
              ...
              >
                <span
                  class="px-1 py-0.5 rounded text-[0.65rem] font-medium"
                  :style="relationBadgeStyle(rel.type)"
                  >{{ rel.type }}</span
                >
                <span class="text-ink-medium">{{ rel.targetPillar }}({{ rel.target }})</span>
...
const historyRecords = ref<
  Array<{ id: number; type: string; input_data: any; created_at: string }>
>([])
...
loadingTimer.value = setTimeout(async () => {
  try {
    ...
    // Compute shensha
    const dayMasterIndex = ['甲', '乙', '丙', '丁', '戊', '己', '庚', '辛', '壬', '癸'].indexOf(
    ...
    try {
      const token = currentProfile.value
...
<div class="relative mt-6">
  <div class="flex items-center justify-center gap-3">
    <!-- Save status -->
    <span v-if="savedDivinationId" class="font-sans text-xs text-wuxing-wood"> 已保存 </span>
    <span v-if="saveError" class="font-sans text-xs text-cinnabar/70"> 保存失败 </span>
