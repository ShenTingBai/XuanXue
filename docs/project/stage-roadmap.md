...
- 完成门槛**尚未满足**：八字来源**取证已完成**（[八字来源台账](../product/evidence/bazi/bazi-source-ledger.md)：8 条来源逐条登记，含国标 GB/T 33661—2017 扫描页目视核对、香港天文台逐年数据逐行核对、日本国立天文台《暦要項》；8 条 GAP 逐条登记），台账状态为 `evidence_prepared_review_pending`。未满足的是**人工审阅裁决**（`sourceReviewStatus` 仍为 `unreviewed`，待 R5-A 审阅），以及台账 §末自列的 3 项 R5-B 前待处理：`GAP-BZ-001`（立春年界为条文推导，需定年柱措辞）、`GAP-BZ-007`（原刻影印核对）、`GAP-BZ-004`（NAOJ 覆盖清单，决定跨年代时刻样例数量）。
