...
- 用户档案、工具统一体验、八字、生肖/太阳星座、日期对照（内部标识 `zeji`）、八字关系对照（现有合婚退役）、称骨表对照（现有称骨算命下线）、周易卦象阅读（现有易经/六爻混合功能退役）、姓名笔画与五格对照（现有姓名测试退役）、测字双轨候选（现有测字占卜退役）和紫微斗数基础命盘（现有紫微预测功能退役）已经形成 `Approved` 产品规范，但这些规范尚未完成系统性代码实施和验收；
...
- 核心三工具的具体过渡策略尚未在单项契约中固定，涉及状态目录的实施阶段需先确定实际范围和目标组合；不得把“核心工具”误解为整工具已经允许公开；
- 未实施的代码问题以审计记录中的 GAP 条目承接。文档整理完成后可以准备一个范围明确的单阶段开发计划，不要求先把全部代码问题修完，也不要求把封存候选的全部来源先查完；
...
下列路线记录此前获批方向。P2 留有历史计划，但用户已经明确暂停讨论和执行实施计划；其状态调整为 `On hold`。P3 起尚未生成可执行计划。任何阶段恢复前，都必须结合现行产品契约重新确认具体范围，不能直接使用旧路线摘要替代最新规范。

### P2：公开真实性收缩

- 状态：`On hold`。历史计划 `.claude/plans/plan-20260901-p2-public-truthfulness-containment.yaml` 尚未执行并原样保留；当前不授权执行，恢复时必须先检查其与现行产品规范和公开试运行边界是否一致。
