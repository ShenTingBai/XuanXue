...
- 状态：`Accepted`（2026-09-20 用户确认；**限定内部验证版**，不含公开放行）。
...
- 实施进展（2026-09-15 核对）：
  ...
  - 2026-09-20 Codex 独立审查、IAB 生产预览和触控目标专项均已完成；限定接受记录见 [R5 内部验证版限定接受](../validation/2026-09-20-r5-limited-acceptance-validation.md)。
- 完成门槛：普通日期部分结果、节气边界候选、页内认证后二次确认、幂等保存、快照读取、重新计算和删除全部通过；八字日期、节气和甲子日来源及黄金样例满足对应准入。
- 来源限制仍保留：`GAP-BZ-001`（立春年界直接原文）与 `GAP-BZ-007`（原刻影印逐句核对）未关闭；这些限制不阻断本次内部验证版接受，但阻断更强的原典核验主张与公开放行。
