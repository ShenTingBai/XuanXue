...
> ⚠️ **本句已在 §15 更正**：该结论**只在重建分支上成立**（`tool-catalog.ts` 11 项全为 `internal`）。
> 线上实际部署的是 `main`（重建前版本），**11 个工具全部公开可达**。据此得出的
> "交付/可用性 3 分"结论方向有误，见 §15.4。
...
- **神煞层面**（⚠️ **复核修正**：以下几项在代码中**均有显式注释声明**，属"已声明的简化/取舍"，
  而非隐藏缺陷。是否与传统典籍一致属**来源核验问题**，须按八字工具契约流程处理，
  **不应由执行方擅自改表**）：
  - `gender` 声明为 `reserved for future use`（`useShenSha.ts:14`）且**未参与计算**
    → 传统上分阴阳男女取用的神煞（如元辰、勾绞）当前实现与任一性别口径都不符；
  - `词馆 = 临官位（同禄神）`（`useShenSha.ts:600` 注释明示）——两个神煞被合并为一个；
  - 天乙/太极/文昌**同时输出日干版与年干版**（`:636` 注释 "coexists with 日干 versions"），
    同名神煞可成对出现，并进入 `useLiuNian.ts:301-304` 的 ±5 累加
    （同一概念在流年评分中可能被计两次）；
  - 元辰/大耗被实现成年支六冲（=岁破）；天赦用月支表却标 `source:'日支'`；
  - 31 个神煞**无任何来源标注**（对比 `constants/bazi-rules.ts` 的逐条 SRC 标注）。
...
| 对象                             | 分     | 理由                                                                                                                                                                                                                                                                            |
| -------------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 八字工具（新引擎，日期级三柱）   | 7/10   | 国标锚点、分钟级节气、边界候选建模正确、证据台账齐全；扣分：只有三柱、节气**时刻**维度只外部核验 2026 一年。⚠️ 本行原文写"全部来源 `unreviewed`"，**已于 §16 更正**：那是 `sourceReviewStatus`（人工审阅流程），不是 `sourceStatus`（证据等级，已给到 `verified_secondary` 等） |
...
> 执行时间：2026-09-15
>
> 授权：用户批准「按推荐顺序执行批次 3，然后接着修复」，批次 4 为纯文档改动
>
> Git：与批次 1/3 同分支提交，**未推送**

### 12.1 完成项

| 文档                            | 改动                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `CLAUDE.md`                     | R4 状态由 `implemented_verification_pending` 更正为 **`Accepted`（2026-09-14）**；composable 清单由 19 个补全为**实际 26 个**，并标注 `useBaZi/useShenSha/useLiuNian` 为旧引擎（日柱锚点已知错误、仍被 `useHeHun` 调用）；组件树补入 `components/bazi/`、`components/profile/`，标注 `components/tools/bazi/` 与 `HistoryModal.vue` 为死代码，**删除已不存在的 `EntertainmentDisclaimer.vue`**；`constants` 树更正 `tool-catalog` 为四维目录、`bazi.ts` 的"唯一数据源"表述、`hehun`/`fortune-sticks` 说明；`server` 树补入 `result-history-schema.ts`、`result-history.ts`、`tool-recompute.ts`、`bounded-json-body`、`instance-lock`，并写明 `db.ts` 的原子落盘与单实例锁；`tests/` 目录清单补全；Git Hooks 表补上 `lint-staged` 与 pre-push 的 typecheck；限流段补上 XFF 从右向左取值；Divinations 段标注为**已退役死代码**并补当前 R5 历史实现；`ZiWeiInputForm` 的 longitude/latitude 表述纠正；CSP 描述与修复后实现对齐 |
| `README.md`                     | 工具治理状态表的 R4 由"已实施（待验收）"更正为 **Accepted**；R4 段落整体重写，补接受依据与"不构成公开放行"限定                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `docs/product/README.md`        | §3.1 当前审阅入口更正；§6.1 矩阵表头日期 `2026-09-09` → `2026-09-15`，`bazi` 行由 `blocked / disabled` 更正为代码事实 **`enabled / create_allowed`**；新增 **§8.7 R5 实施状态**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `docs/project/stage-roadmap.md` | §3 状态汇总由"R5 尚未生成实施计划"更正为"R5 已进入实施"；R5 段补实施进展、已提交/未提交清单、完成门槛尚未满足的说明                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |

### 12.2 刻意**没有**改的部分（边界声明）

1. **`docs/product/README.md` §8.1–§8.6 的历史叙述保持原状。** 该文档自己的规则是
   「旧决策发生变化时新增 ADR 或明确标注 `Superseded`，不通过改写历史制造一致性」，
   因此 §8.6 中"R5 尚未生成实施计划"作为 2026-09-14 当时的记录予以保留，
   当前状态另立 §8.7。
2. **未把 R5 写成 `Accepted`。** 阶段状态是用户与 Codex 的治理权限。
   本次只写入**可核对的代码事实**（工具目录枚举、已提交/未提交清单、已知缺陷），
   并明确标注"R5 阶段状态仍由用户确认"。
3. 未改动 `docs/design/design-system.md`（属用户未提交的 R5-D 工作范围）。

### 12.3 验证结果

| 检查                                | 结果                                                                              |
| ----------------------------------- | --------------------------------------------------------------------------------- |
| 是否有测试读取这 4 个文件           | **0 命中**（grep `README.md` / `stage-roadmap` / `CLAUDE.md` 于 `tests/**`）      |
| `npx prettier --check .`（全仓）    | 通过                                                                              |
| 提交树与已验证树（`928358b`）的差异 | **仅这 4 个 markdown 文件**；代码树完全相同，故 85 文件 / 2627 用例的结论原样成立 |
...
| ------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 批次 2（正确性）   | 旧引擎日柱锚点修正、`useHeHun` 切换到 `utils/bazi`、重写被固化的 inline snapshot、`calculateShenSha` 补 `gender` 维度、修词馆=禄神与天乙/太极/文昌双份重复 |
| 批次 5（可维护性） | 拆巨型函数、统一地支关系与五行映射数据源、处置旧八字栈死代码、开启 type-aware lint、grep 测试转行为测试、DB 测试隔离                                       |
...
**根因**：`composables/useBaZi.ts` 的 `getDayPillarIndices()` 用 Zeller 式公式配一个**错误锚点**，
注释断言"1900-01-01 是甲子日"，**真值为甲戌**。由于天干与地支是对同一个偏差值分别取模，
锚点偏 10 位表现为**日干恒对、日支恒偏移 +2**（所有日期的日支都错）。
日支为夫妻宫，直接决定合婚的六合/六冲/六害/相刑与空亡取旬。

**验证方式（改前）**：两种独立方法对 7 个日期交叉比对，结果完全一致——
① 国标锚点 1949-10-01 = 甲子 的纯整数儒略日差取模；② `lunar-javascript` 的 `getDayInGanZhi()`。
并与外部来源的黄金样例 BZ-703（1964-07-14 = 甲子）、BZ-704（2000-01-01 = 戊午）吻合。

**修复方式**：新增 `dayGanZhiIndex(year, month, day)` 到 `utils/bazi/pillars.ts`，
`dayGanZhi()` 改为调用它；旧引擎 `getDayPillarIndices()` 直接复用同一函数。
**日柱规则在仓库内只保留一处实现**，不再存在第二个可以漂移的锚点。
该入口接受任意整数年月日、不做公历合法性校验，以保持 `calculateBaZi` 既有的输入语义
（严格校验由 `utils/bazi` 承担）。

**`useHeHun` 无需迁移**：它经 `calculateBaZi` 取得 `dayPillar.branch`，
锚点修好后自动获得正确日支，因此本批**没有**做整体引擎切换（新引擎目前只出三柱，
不产出合婚需要的时柱，整体迁移属独立任务）。

### 13.2 被修复暴露的测试问题（原夹具建立在错误日柱上）

| 位置                              | 问题                                                                           | 处理                                                                          |
| --------------------------------- | ------------------------------------------------------------------------------ | ----------------------------------------------------------------------------- |
| `useBaZi.test.ts` 2000-01-01 快照 | 日支冻结为 `申`（正确 `午`）；同文件 `:832` 注释却写着"戊午日"——注释对、快照错 | 更正为 `午`                                                                   |
| `useBaZi.test.ts` 1964-07-14 快照 | 日支冻结为 `寅`（正确 `子`）                                                   | 更正为 `子`                                                                   |
| `useBaZi.test.ts` 2004-02-05 快照 | 日支冻结为 `辰`（正确 `寅`）                                                   | 更正为 `寅`                                                                   |
| `useBaZi.test.ts` 元素统计快照    | 随日柱由壬戌→壬申而变化                                                        | 更正，并**逐项推导验证**（土 −1 / 火 −1 / 金 +2 / 水 0 / 木 0，总数 24 不变） |
| `useBaZi.test.ts` 藏干测试        | 夹具用 1964-07-14 并假定"甲寅日"，实为甲子（子藏癸）                           | 改用 **1960-01-27（甲寅日，已验证）**，保留原意                               |
| `useShenSha.test.ts` 禄神用例     | 同样假定 1964-07-14 为甲寅日                                                   | 同上改用 1960-01-27                                                           |
| `useShenSha.test.ts` 华盖用例     | 假定 1998-05-25 日支为戌，实为申（全盘无戌）                                   | 改用 **1998-02-08（戊寅年丙戌日，已验证）**                                   |

**这正是审计所指出"测试把错误固化成期望"的实例**：修复根因后，7 处依赖错误日柱的断言
同时暴露。所有更正后的期望值均来自独立验证，而非"接受改后的输出"。

### 13.3 刻意**未**做：神煞规则改写（附复核修正）

`CLAUDE.md` 规定"神煞查找表只证明当前项目如何实现，不是权威来源。**未经对应工具契约和来源核验，
不得把映射写成已验证规则**；修改时必须保留规则版本和审计证据"。

复核代码后确认，先前列为问题的三项**均有显式注释声明**，属"已声明的简化/取舍"而非隐藏缺陷：

- `gender`：`useShenSha.ts:14` 标注 `reserved for future use`；
- `词馆 = 临官位（同禄神）`：`:600` 注释明示；
- 天乙/太极/文昌日干版 + 年干版并存：`:636` 注释明示 "coexists with 日干 versions"。

因此**本批不改动任何神煞映射**。这些属来源核验流程（八字工具契约），不是执行方权限。
除已在 §6.7 记录的内容外，无新增代码改动。
...
| 改动文件            | `utils/bazi/pillars.ts`、`composables/useBaZi.ts`、`tests/composables/{useBaZi,useShenSha}.test.ts` |

### 13.5 仍未解决

1. **`useHeHun` 整体迁移到 `utils/bazi` 未做**：新引擎目前只产出三柱，不产出合婚所需的时柱；
   整体迁移需先扩展新引擎，属独立任务。
...
## 十五、线上部署事实与暂缓决定（2026-09-15）

> 本节记录一次**只读核查**发现的运行环境事实，以及用户对该事实的决定。
> 不是代码缺陷记录，也不构成整改承诺。

### 15.1 事实：线上跑的是重建前的 `main`

经核对，公网服务器部署自 **`main`** 分支。该分支尖端为 `2aa1574`（**2026-06-08，距今 102 天**），
**领先/落后关系为：落后 `codex/foundation-rebuild` 26 个提交**。

`main` 上**完全不存在** R1–R5 的任何成果：

| 能力                                      | `main` 现状                                                                           |
| ----------------------------------------- | ------------------------------------------------------------------------------------- |
| R1 工具四维围栏                           | `constants/tool-catalog.ts` 与 `middleware/tool-availability.global.ts` **均不存在**  |
| R2 账号会话重建                           | 旧 auth 仍在；**`server/api/profiles/*` 五个接口仍可写**（未 410）                    |
| R3 生肖                                   | 无                                                                                    |
| R4 本人档案                               | `self-profile-schema.ts` / `services/self-profile.ts` / `api/self-profile/*` **全无** |
| R5 八字与结果历史                         | `result-history-schema.ts` 等**全无**                                                 |
| 本报告批次 1/3 的 4 项安全修复 + 单实例锁 | **全部没有**                                                                          |

**线上因此处于 P0 围栏之前的状态**：11 个工具公开可达（六月版，含工程评分与未核验来源）、
旧档案接口可写、占卜结果 fire-and-forget 自动保存。

### 15.2 线上确实存在的四项缺陷（逐条在 `main` 源码中核对）

| 缺陷                            | `main` 源码证据                                                                 | 潜在后果                                                          |
| ------------------------------- | ------------------------------------------------------------------------------- | ----------------------------------------------------------------- |
| CSP nonce 从未生效              | `server/plugins/csp.ts` 仅读 `response.headers?.['Content-Security-Policy']`    | 实际生效 `script-src 'unsafe-inline'`                             |
| **XFF 取最左值**                | `server/utils/rateLimit.ts`：`forwarded.split(',')[0].trim()`                   | 若在 nginx 之后，**登录限流可被伪造 XFF 完全绕过 → 无限在线撞库** |
| **认证端点只信 Content-Length** | `login.post.ts`：`parseInt(getHeader('content-length') \|\| '0')` 后 `readBody` | `chunked` 绕过上限，**未认证单请求即可耗尽内存**                  |
| **`saveFile` 非原子整文件覆盖** | `db.ts`：`db.export()` → `fs.writeFileSync(path)`                               | 写入中崩溃/断电即整库损毁；无实例锁，多实例互相覆盖               |

### 15.3 用户决定（2026-09-15）

用户明确决定：**暂不处理线上部署**，继续本地重建工作。

该决定被记录而非淡化，理由：这是**已发现的已知风险**，将来站点一旦对外宣传或改变用途，
本节应作为起点重新评估。核查中同时确认：线上 URL 的对外可见范围、是否在反向代理之后、
代理是否使用 `$proxy_add_x_forwarded_for` 均**未核实**——三者直接决定 §15.2 相关缺陷的
可利用性。

### 15.4 对本报告结论的更正

本报告 §6.1 与 §8 中"**产出 0 个公众可用工具**"及据此给出的"**交付/可用性 3 分**"，
**只在重建分支上成立**。就线上实际运行的系统而言，11 个工具全部公开可用。

因此更准确的判断是：**本项目的核心问题不是"交付不足"，而是"正在构建的系统与实际
运行的系统是两套"** —— 重建分支上所有"0 个公开工具"的收敛，在线上并不存在。
§8 的"交付/可用性"评分相应不成立，应以本节为准。

---

## 十六、更正：八字来源**取证已完成**，缺的是审阅裁决（2026-09-15）

> 本节更正本报告 §8 评分表中"全部来源 `unreviewed`"的表述。该表述**字面正确但严重误导**，
> 经用户指出后回查台账原文确认。

### 16.1 实际完成的工作

[八字来源台账](../../product/evidence/bazi/bazi-source-ledger.md)（71 KB，2026-09-14，R5-A 初稿）
记录的工作量为：

- **8 条来源逐条登记**：`SRC-BZ-001` 国标 GB/T 33661—2017、`SRC-BZ-002` 香港天文台历表、
  `SRC-BZ-003` 日本国立天文台《暦要項》、`SRC-BZ-004/005` HKO 干支纪年与节气黄经、
  `SRC-BZ-006` lunar-javascript（标为 `implementation_only`）、`SRC-BZ-008/009`《三命通会》等；
- **国标扫描页目视核对**：把目标页解码为图像后逐页核对
  （§5.2、§6.1.1、§6.2、§6.3.1、§6.3.2、§6.4、§7.1—7.3，印刷页 4 / PDF 页 10；附录 A 表 A.1，印刷页 5 / PDF 页 11）；
- **HKO 逐年数据下载逐行核对**：`T2000c.txt`（十二个"节"全部采样）、`T2023c.txt`（闰二月）、`T2026c.txt`；
- **8 条 GAP 逐条登记**：含已关闭的 `GAP-BZ-002`（五虎遁已找到《三命通会》古歌原文）、
  收敛未关闭的 `GAP-BZ-001`、以及 5 条待处理项；
- **如实记录取源环境限制**：`web_search` endpoint 404、`web_fetch` 不支持 PDF，
  因此改为本地下载 + `pypdf` + 自写 PNG 解码目视核对，并**未安装任何新依赖**。

台账自身状态：**`evidence_prepared_review_pending`（R5-A 待审阅）**。

### 16.2 被本报告混淆的两个独立维度

治理规范 §16.2 把二者分开定义，且明确规定互不替代：

| 维度                 | 含义             | 八字实际值                                                                                              |
| -------------------- | ---------------- | ------------------------------------------------------------------------------------------------------- |
| `sourceStatus`       | **证据支持程度** | ✅ 已给定级（国标为 `verified_secondary`，扫描页已目视核对；lunar-javascript 标 `implementation_only`） |
| `sourceReviewStatus` | **人工核验流程** | ⚠️ `unreviewed` —— 待 R5-A 审阅裁决                                                                     |

本报告 §8 原文"全部来源 `unreviewed`"只对第二列成立；**读起来却像"资料没查"，
与第一列已完成的定级事实相反**。规范中"`approved` 也不能替代 `verified_primary` 或
`verified_secondary`"一句在反方向同样成立：**等级已给 ≠ 审阅已过，但等级已给绝不等于"没做"**。

### 16.3 剩余工作不是开放式研究，台账已列成清单

台账 §末「R5-B 之前必须处理」已明确三项：

| 缺口         | 内容                                                                                                            | 待决定                                                                |
| ------------ | --------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------- |
| `GAP-BZ-001` | 立春作为八字年柱边界**没有一句直接宣称它的古籍原文**，现依据为两条条文推导                                      | **年柱措辞**：只能表述为"由条文推导 + 项目采用"，不得写"古籍明文规定" |
| `GAP-BZ-007` | `SRC-BZ-008/009` 载体为在线整理本（底本未核实）；原刻影印核对未完成（内阁文库旧链接 404、早稲田检索参数未打通） | 传统规则维度能否脱离"在线整理本层级"                                  |
| `GAP-BZ-004` | NAOJ HTML 版年份覆盖清单未逐一确认                                                                              | 跨年代节气**时刻**样例可取多少个                                      |

另 `GAP-BZ-005`（1901—2005 缺可抓取的 A 级分钟级来源）已由台账收敛为：
日期级 A 级核验为全覆盖（HKO 文本版 1901—2100 逐日），**时刻**维度只能抽样。

### 16.4 教训

诊断"项目缺什么"时，**必须区分"证据等级未给"与"审阅流程未走"** ——
两者在状态枚举里是两个字段，在改进建议里却是完全不同的行动：
前者需要取证，后者只需要一次审阅签字。把它们混为一谈，会**把已经做完的工作说成没做**，
并据此虚构出一大块并不存在的待办。
