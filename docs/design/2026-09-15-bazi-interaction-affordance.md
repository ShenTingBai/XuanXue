...
不做：不动 `items` / `footnote` / 锚点行为 / IntersectionObserver / `data-profile-index` 钩子（两个档案页测试依赖它）；不改窄屏「转正文上方网格」的结构。
...
### H 卷目高亮改为「每次按几何重算」（2026-09-15 追加，用户批准）
...
**改法**（`components/editorial/IndexNav.vue`，仅脚本）：抽出 `syncActive()`，每次回调按当前几何重算全集——优先取**完全覆盖高亮带**（视口 25%–35%）的那一节，否则取最靠上的相交节，再不然取最后一节顶边已在带上方的（滚到底时保持末节高亮）；`onMounted` 注册观察后先对齐一次。
