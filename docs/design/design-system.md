...
当前梅花旧页面用此纸张渐变卡承载“白话解读”。这里只保留视觉资产记录，不批准继续输出综合解卦或未来预测。目标内容遵守[梅花易数·起卦演示契约](../product/contracts/meihua-tool-contract.md)：默认展示推导过程，本卦与变卦窄屏纵向排列，原文和项目解释分层，互卦/体用为次级展开资料，来源进入正常阅读流。现有 UI 尚未按此整改。
...
#### 八字页专用类与组件（`/tools/bazi`，2026-09-15 R5-C）

设计基线：`docs/design/2026-09-15-bazi-ui-spec.md`（§1 六段结构、§3 状态矩阵 13 态、§4 响应式与可访问性、§5 令牌与类）。八字页的六段顺序、内容闭集与措辞红线由该规格与治理规范 §4 共同约束，组件不得自行改写限制与精度声明。

**全局类**（新增于 `assets/css/main.css` 的 `@layer components` 内，与本记录同批）：

| 类                         | 用途                                                               | 边界                                                                                    |
| -------------------------- | ------------------------------------------------------------------ | --------------------------------------------------------------------------------------- |
| `bazi-pillar-summary`      | Ⅲ 段三柱一览容器：三柱横排；≤389px 转纵排（窄屏保可读）            | 只管布局；柱字字号由组件按规格给出（桌面最大、移动端 1.9rem 一档）                      |
| `bazi-pillar-cell`         | 单柱格：居中、细线框、暖白底                                       | 不写死五行色；五行色由组件按 `WUXING_COLORS` 绑定，回退 `WUXING_FALLBACK_COLOR`         |
| `bazi-pillar-cell--day`    | 日柱修饰：更实描边 + 极浅洗底                                      | 与年/月柱的区分来自边框与底纹，**不来自颜色**（不得仅靠颜色区分）                       |
| `bazi-pillar-wuxing`       | 五行标注行：等宽数字 + 小字号                                      | 不负责上色；`font-mono` 需求以 `tabular-nums`（`.editorial-num`）实现，不新增第三个字族 |
| `bazi-candidate-lane`      | Ⅳ 段候选轨道：中性洗底 + 细线框                                    | **禁止红/绿区分**；「甲 / 乙」必须以文字标注                                            |
| `editorial-dialog-confirm` | 危险/重要确认块：朱砂描边 + 文字说明（清空确认、保存前的隐私提示） | **不使用整块红底**；严重性靠文字与描边表达                                              |

上述类的颜色一律用 `color-mix(...)` 或既有令牌，禁止 `rgba()` 与 `bg-x/10` 斜杠写法（§2.6）。

**组件**（`components/bazi/*`）：

| 组件                     | 段  | 职责                                                                                                                 |
| ------------------------ | --- | -------------------------------------------------------------------------------------------------------------------- |
| `BaziInputForm`          | Ⅱ   | 历法切换、年月日、闰月显式选择、就地说明、十四周岁声明；禁用时把原因写在按钮旁                                       |
| `BaziStatusBanner`       | Ⅲ   | 四类状态横幅（partial / candidate / stale / 失败），**文字 + 图标 + 颜色三重编码**                                   |
| `BaziPillarCard`         | Ⅳ   | 单柱：干支、天干/地支五行、日干标注；详情进原生 `details`，不用固定 max-height                                       |
| `BaziCandidatePanel`     | Ⅳ   | 跨节候选：恰好两个完整情形，文字标注「甲 / 乙」，不只靠颜色                                                          |
| `BaziDateComparison`     | Ⅳ   | 日期对照：分清「你填写的」与「依规则换算的」                                                                         |
| `BaziReadingGuide`       | Ⅳ   | 六问三层折叠（问题 / 解释 / 依据）                                                                                   |
| `BaziElementComposition` | Ⅳ   | 五行构成：**只统计六个字的字面出现次数**；必须写明不代表旺衰、强弱、平衡与否，也不用于喜忌判断；不得出现百分比或比例 |
| `BaziEvidenceScope`      | Ⅴ   | 来源（带等级）、版本、限制说明、未输出清单、内容标签；文案取自 `constants/bazi-rules.ts`                             |
| `BaziSaveDialog`         | Ⅵ   | 保存确认弹层：先展示七项摘要再确认；焦点进入 / Escape / Tab 不外溢 / 焦点返回                                        |
| `BaziHistoryPanel`       | Ⅵ   | 历史列表（不含出生日期）、快照只读、用当前规则重算、删单条、按条数清空                                               |
