...
export function shenShaBadgeStyle(category: '吉' | '中性' | '凶'): {
  bg: string
  text: string
  border: string
} {
  if (category === '吉')
    return {
      bg: `color-mix(in srgb, ${WUXING_COLORS['木']} 8%, transparent)`,
      text: WUXING_COLORS['木'],
      border: `color-mix(in srgb, ${WUXING_COLORS['木']} 25%, transparent)`,
    }
  if (category === '凶')
    return {
      bg: `color-mix(in srgb, ${WUXING_COLORS['火']} 8%, transparent)`,
      text: WUXING_COLORS['火'],
      border: `color-mix(in srgb, ${WUXING_COLORS['火']} 25%, transparent)`,
    }
  return {
    bg: `color-mix(in srgb, ${WUXING_FALLBACK_COLOR} 8%, transparent)`,
    text: WUXING_FALLBACK_COLOR,
    border: `color-mix(in srgb, ${WUXING_FALLBACK_COLOR} 25%, transparent)`,
  }
}
