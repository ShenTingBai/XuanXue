...
const mockGetHeader = vi.hoisted(() => vi.fn((_event: any, name: string) => {
...
describe('Profiles API handlers', () => {
  ...
  describe('GET /api/profiles (list)', () => {
    ...
    it('returns subs as an array of profiles with isMain=false', async () => {
      vi.mocked(dbGet).mockReturnValue({
        id: 1, nickname: 'mainuser',
        created_at: '2025-01-01T00:00:00.000Z', updated_at: '2025-01-01T00:00:00.000Z',
      })
      vi.mocked(dbAll).mockReturnValue([
        { id: 2, nickname: 'sub1', created_at: '2025-01-02T00:00:00.000Z', updated_at: '2025-01-02T00:00:00.000Z' },
        { id: 3, nickname: 'sub2', created_at: '2025-01-03T00:00:00.000Z', updated_at: '2025-01-03T00:00:00.000Z' },
      ])
    ...
    it('throws 404 when main profile not found', async () => {
      ...
      await expect(handler({ context: { profileId: 999 } } as any)).rejects.toMatchObject({ statusCode: 404 })
    })
  })
  ...
  describe('POST /api/profiles (create sub-profile)', () => {
    ...
    beforeEach(async () => {
      ...
      vi.mocked(dbGet).mockImplementation((sql: string) => {
        ...
        if (sql.includes('SELECT * FROM profiles WHERE id =')) return {
          id: 2, nickname: 'newsub',
          created_at: '2025-01-01T00:00:00.000Z', updated_at: '2025-01-01T00:00:00.000Z',
        }
        return undefined
      })
    ...
    it('throws 409 when nickname already exists', async () => {
      ...
      await expect(handler({ context: { profileId: 1 } } as any)).rejects.toMatchObject({ statusCode: 409 })
    })
  ...
  describe('GET /api/profiles/[id] (detail)', () => {
    ...
    it('throws 401 without auth header', async () => {
      await expect(handler({ context: { params: { id: '1' } } } as any))
        .rejects.toMatchObject({ statusCode: 401 })
    })

    it('throws 403 when profileId does not match requested id', async () => {
      await expect(handler({ context: { params: { id: '2' }, profileId: 1 } } as any))
        .rejects.toMatchObject({ statusCode: 403 })
    })

    it('throws 404 when profile does not exist', async () => {
      vi.mocked(dbGet).mockReturnValue(undefined)
      await expect(handler({ context: { params: { id: '1' }, profileId: 1 } } as any))
