...
**玄·道** 将八字、紫微斗数、六爻、生肖、星座等传统术数搬上 Web。用户只需填写一次出生信息，即可在 11 个工具中自由探索——所有计算在浏览器端完成，无需等待后端响应。
...
> 运行 `npm run dev` 后访问 `http://localhost:3000` 即可体验。首页展示全部工具列表和今日玄机；登录后可保存档案、查看历史记录。
...
| **择日** | 建除十二星 + 二十八宿 + 黄历 | 日历视图、吉日推荐、宜忌事项                             |
...
项目采用 Vitest 进行单元测试，覆盖所有计算引擎和服务端逻辑。

```bash
npm run test           # 运行全部测试（36 个测试文件，2000+ 测试用例）
...
```
XuanXue/
├── composables/        # 21 个计算引擎，纯函数为主
├── constants/          # 命理数据：干支、卦象、星曜、笔画字典等（20 个文件）
├── components/
│   ├── home/           # 首页独占组件
│   └── tools/          # 11 个工具 × 各自组件 + 通用组件（ToolPageLayout 等）
├── pages/              # 15 个页面：首页 / 登录 / 隐私 / 档案 / 11 个工具
├── server/
