<template>
  ...
  >
    ...
    <p class="font-sans text-xs text-ink-medium mb-5">
      本结果为算法推演，仅供参考，不能替代专业命理师分析。
    </p>
    ...
    <div class="space-y-5 font-sans text-base text-ink-medium leading-relaxed">
      ...
      <div>
        ...
        <p class="mt-2">
          ...
          <template v-if="unfavorableElements.length > 0">
            而<strong class="text-ink-dark">{{ unfavorableElements.join('、') }}</strong
            >与你相克， 适当平衡即可，不必刻意回避。
          </template>
        </p>
      </div>
      ...
      <div v-if="readingGuideShensha.length > 0">
        ...
        <div class="flex flex-wrap gap-2 mb-2">
          ...
          >
            ...
            <span
              class="text-xs"
              :style="{ color: 'color-mix(in srgb, var(--color-ink-muted) 60%, transparent)' }"
              >{{ shen.pillar }}</span
            >
          </span>
        </div>
        <p class="text-sm">
          <template v-for="(shen, i) in readingGuideShensha" :key="shen.name + shen.pillar">
            <strong
              ...
              "
              >{{ shen.name }}</strong
            >（{{ shen.pillar }}）：{{ shen.description
            }}<template v-if="i < readingGuideShensha.length - 1">；</template>
          </template>
        </p>
      </div>
      ...
      <div v-if="currentYearLiuNian">
        <h3 class="font-sans text-sm font-medium text-ink-dark mb-2">
          今年运势（{{ currentYearLiuNian.year }}年）
        </h3>
        <div class="flex items-center gap-3 mb-2">
          ...
          <span
            class="text-xs"
            :class="
              currentYearLiuNian.isFavorable
                ? 'text-wuxing-wood'
                : currentYearLiuNian.isUnfavorable
                  ? 'text-cinnabar'
                  : 'text-ink-medium'
            "
          >
          ...
          <span class="ml-auto font-sans text-xs text-cinnabar font-medium"
            >运势评分 {{ currentYearLiuNian.score }}/100</span
          >
        </div>
        ...
        <div v-if="currentYearLiuNian.earthRelations.length > 0" class="mt-1.5">
          ...
          <ul class="mt-0.5 space-y-0.5 text-xs text-ink-medium">
            <li v-for="(rel, i) in currentYearLiuNian.earthRelations" :key="i">
              <span class="font-medium">{{ rel.targetPillar }}{{ rel.type }}</span
              >：{{ rel.description }}
            </li>
          </ul>
        </div>
        <p
          v-if="currentYearLiuNian.detail?.daYunInteraction"
          class="mt-1.5 text-xs text-ink-medium"
        >
          {{ currentYearLiuNian.detail.daYunInteraction }}
        </p>
      </div>
      ...
      <div v-if="currentDaYun">
        ...
        <div class="flex items-center gap-3 mb-1">
          ...
          <span class="text-xs text-ink-medium"
            >{{ currentDaYun.startAge }}岁 - {{ currentDaYun.endAge }}岁</span
          >
        </div>
        <p class="text-sm">
          {{ currentDaYun.description }}，{{ getDaYunMeaning(currentDaYun.stemTenGod) }}
        </p>
      </div>
        ...
        <template v-if="unfavorableElements.length > 0">
          ...
          <div class="grid grid-cols-2 sm:grid-cols-3 gap-2 text-xs">
            ...
            >
              <span
                class="font-medium"
                :style="{ color: `color-mix(in srgb, ${ELEMENT_COLORS[el]} 70%, transparent)` }"
                >{{ el }}</span
              >
