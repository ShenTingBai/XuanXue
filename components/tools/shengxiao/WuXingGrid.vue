<template>
  <div class="fade-in mb-6" :style="{ '--delay': '0.15s' }">
    ...
    <p class="mt-3 font-sans text-xs text-ink-medium leading-relaxed">
      纳音为古乐律十二律吕配六十甲子，{{ result.stemBranch }}年属「<span
        class="text-cinnabar font-medium"
        >{{ result.naYin }}</span
      >」。
      <template v-if="nayinWuxing !== result.wuXing">
        其五行属<span class="text-cinnabar font-medium">{{ nayinWuxing }}</span
        >，与生肖五行<span class="text-cinnabar font-medium">{{ result.wuXing }}</span
        >有别——纳音定命格底色，生肖定外显气质。
      </template>
      <template v-else>
        其五行与生肖五行同为<span class="text-cinnabar font-medium">{{ result.wuXing }}</span
        >，内外一致，表里如一。
      </template>
    </p>
    ...
    <div class="mt-5 pt-4 border-t border-ink-faint/15">
      ...
      <div class="grid grid-cols-3 gap-3">
        <div class="text-center">
          <div class="font-display text-base text-ink-dark mb-0.5">
...
const nayinWuxing = computed(
  () =>
    getNayinWuxing(props.result.heavenlyStem, props.result.earthlyBranch) || props.result.wuXing,
)
