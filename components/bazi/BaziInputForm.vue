 ...
 * - 触控目标 ≥44px，窄屏与文字放大时不产生横向滚动；颜色不是唯一区分手段。
 */
...
<template>
  <fieldset :aria-describedby="error ? errorId : undefined">
    ...
    <div class="mt-2" role="radiogroup" aria-label="历法选择">
    ...
    >
      ...
      <p v-if="draft.isLeapMonth === null" class="mt-2 font-sans text-xs text-ink-medium">
    ...
    <!-- 十四周岁声明（数据规范 §5.4） -->
    <label class="mt-4 flex items-start gap-3 font-sans text-sm text-ink-medium leading-relaxed">
      <input
        type="checkbox"
        class="mt-1"
    ...
    <p v-if="error" :id="errorId" class="mt-3 font-sans text-sm text-cinnabar" role="alert">
      {{ error }}
    </p>
  </fieldset>
</template>
